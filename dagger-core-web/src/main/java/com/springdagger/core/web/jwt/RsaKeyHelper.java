...
public class RsaKeyHelper {
     ...
     * @throws Exception
     */
    public PublicKey getPublicKeyByPath(String filename) throws Exception {
     ...
     * @throws Exception
     */
    public PrivateKey getPrivateKeyByPath(String filename) throws Exception {
        InputStream resourceAsStream = this.getClass().getClassLoader().getResourceAsStream(filename);
        DataInputStream dis = new DataInputStream(resourceAsStream);
        byte[] keyBytes = new byte[resourceAsStream.available()];
        dis.readFully(keyBytes);
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException {
        KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance("RSA");
        SecureRandom secureRandom = new SecureRandom("htbx_life_dev".getBytes());
//        SecureRandom secureRandom = new SecureRandom("htbx_life_test".getBytes());
//        SecureRandom secureRandom = new SecureRandom("htbx_life_prod".getBytes());
        keyPairGenerator.initialize(1024, secureRandom);
        KeyPair keyPair = keyPairGenerator.genKeyPair();
        String publicKeyStr = Base64Utils.encodeToString(keyPair.getPublic().getEncoded());
        String privateKeyStr = Base64Utils.encodeToString(keyPair.getPrivate().getEncoded());
